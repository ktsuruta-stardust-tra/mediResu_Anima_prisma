    ...
    <main className="flex flex-col items-center w-full bg-[#d9ecec]">
        ...
        <div className="flex flex-col w-full max-w-[375px] items-start gap-2.5 p-5 bg-[#d9ecec]">
            <div className="flex flex-col items-start px-0 py-[5px] w-full bg-white rounded-md">
                <ItemsAnyComp className="!flex-[0_0_auto] mt-" text="保険治療の経験" />
                <SubItemsComp
                ...
                />
                {categories.filter(category=> category.id >= 5).map((category) => (
                    <div key={category.id}>
                        <ChecklistSection 
                          key={category.id} 
                          title={category.name} 
                ...
                <BackEndComp className="w-full" img="/img/userinfo/subtract-1.svg" subtract="/img/userinfo/subtract-6.svg" text="次へ" backLink="../category-step1" nextLink="../experience" />
            </div>
        </div>
    </main>
  );
