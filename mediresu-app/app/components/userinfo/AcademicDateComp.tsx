      ...
      <div className={`flex w-[335px] items-center pt-2.5 pb-0 px-5 relative ${className}`}>
        <div className="flex w-[100px] gap-1 self-stretch items-center relative">
          ...
          <select 
            ...
            onChange={(e) => handleChange(formData?.order_num || 0,"education_start_year",e.target.value)}
            >
        ...
        <select 
          ...
          onChange={(e)=>handleChange(formData?.order_num || 0,"education_start_month",e.target.value)}
          >
      ...
      <div className={`flex w-[335px] items-center pt-2.5 pb-0 px-5 relative ${className}`}>
        <div className="flex w-[100px] gap-1 self-stretch items-center relative">
          <select 
            ...
            onChange={(e) => handleChange(formData?.order_num || 0,"education_end_year",e.target.value)} 
            >
            <option value="">年を選択</option>
            {years.map((y) => (
              <option key={y} value={y}>
        ...
        <select 
          ...
          onChange={(e) => handleChange(formData?.order_num || 0,"education_end_month",e.target.value)}
          >
          <option value="">選択</option>
          {months.map((m) => (
            <option key={m} value={m}>
