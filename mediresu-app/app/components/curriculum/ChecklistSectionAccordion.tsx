...
// セクションコンポーネント
export const ChecklistSectionAccordion = ({
  title,
  itemCount,
  operations,
  selectedItems,
  ...
  return (
    <div className="w-[335px] items-center flex flex-col relative flex-[0_0_auto]">
      {/* ここにクリックイベントを追加してアコーディオンを開閉 */}
      <div 
        className="flex flex-col items-center relative self-stretch w-full flex-[0_0_auto]"
        onClick={() => setIsOpen(!isOpen)} // クリックで開閉
      >
      ...
      {isOpen && (
        <div className="flex flex-col w-[295px] items-start gap-2.5 px-5 py-[21px] relative flex-[0_0_auto] bg-white rounded-[0px_0px_4px_4px] border border-solid border-[#24b6ae]">
            ...
            <div key={operation.id}>
              <div className="flex w-[255px] h-[30px] items-center gap-2.5 px-0 py-2.5 relative">
                <label className="inline-flex items-center cursor-pointer gap-2">
                  <input
                    type="checkbox"
                    className="hidden peer w-[30px] h-[30px]"
                    name="operations"
                    value={operation.id}
          ...
          <button className="flex h-[50px] items-center justify-center gap-1.5 relative self-stretch w-full bg-[#d9ecec] rounded active:scale-95 transition-transform duration-150 ease-in-out"
              onClick={() => setIsOpen(false)}>
            <div 
              className="relative w-[30px] h-[30px] bg-[#24b6ae] rounded-[15px]">
